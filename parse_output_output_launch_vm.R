#!/usr/bin/Rscript
old=getwd();
basedir="/home/peipei/course/CSC547/experiment_docker_hadoop/output_launch_vm";
setwd(basedir);

#file_patterns=c("output_empty.txt.*.dat$","output_empty_2.txt.*.dat$","output_empty_3.txt.*.dat$")
file_patterns=c("output_launch_vm_1.txt.*.dat$","output_launch_vm_2.txt.*.dat$","output_launch_vm_3.txt.*.dat$")
XC=list();
...
for(col_num in 1:length(X)){
  ...
  name=paste("output_launch_vm",name1,sep=".");
  ...
  # print(average)
  # print(value1)
  plot(x=x_axis,y=NULL,ylim=c(v_min,v_max),type="n",main="Overhead of launching VM",xlab="time(seconds)",ylab=metrics_meaning[[name1]],col="black");
