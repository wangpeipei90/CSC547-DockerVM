...
fs_docker=c("output_launch_docker_1.txt","output_launch_docker_2.txt","output_launch_docker_3.txt");
...
fs_vm=c("output_launch_vm_1.txt","output_launch_vm_2.txt","output_launch_vm_3.txt");
...
plot(x=NULL,y=NULL,mai="Overhead comparison of launching Docker container and VM",xlab="Percentage of CPU usage(%)",ylab="Percentage of MEM usage(%)",xlim=range(0:26),ylim=range(28:34));
...
sample=1;
x_cpu=as.vector(XC_docker[[sample]][[cnames_docker_cpu[sample]]]);
y_mem=as.vector(XC_docker[[sample]][[cnames_docker_mem[sample]]]);
