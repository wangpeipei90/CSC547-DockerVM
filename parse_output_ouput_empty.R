#!/usr/bin/Rscript
old=getwd();
basedir="/home/peipei/course/CSC547/experiment_docker_hadoop/output_empty";
setwd(basedir);
...
for(col_num in 1:length(X)){
 ...
 # avg_e=c(sum(value1)/length(value1),sum(value2)/length(value2),sum(value3)/length(value3));
  #max_e=c(max(value1),max(value2),max(value3));
  ...
  # print(average)
  # print(value1)
  #print(metrics_meaning[[name1]]);
  plot(x=x_axis,y=NULL,ylim=c(v_min,v_max),type="n",main="Overhead of SAR without container or VM launched",xlab="time(seconds)",ylab=metrics_meaning[[name1]],col="black");
